...
fu! RunDb()
    ...
    if g:db_rst_mode > 0 | let opts['silent'] = 1 | en
    cal asyncrun#run(0, opts, printf('sqlite3 %s ".read %s"', g:db_path, fnamemodify(g:db_script, ':t')))
endf

fu! ResultToTab()
    if g:db_rst_mode > 0 && match(g:asyncrun_info, '^sqlite3') >= 0 && g:asyncrun_status == 'success'
...
au User AsyncRunStop :cal ResultToTab()
